...
- A test enumerates every endpoint of each host that accepts POST, PUT, PATCH or
  DELETE and is not on the SC-4 exemption list, sends each a request without an
  antiforgery token as a user with an allowed role (anonymous forms anonymously),
...
- The session and antiforgery cookies of each host are asserted to carry the
  `httpOnly`, `Secure` and `SameSite` attributes SC-2 fixes for that host, and
  every other cookie is asserted to be `Secure`.
