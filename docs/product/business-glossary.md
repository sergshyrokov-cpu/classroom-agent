...
| **MeetingCodeLink** | привязка кода встречи | Links a Meet meeting code to a course — created automatically or by a Dean or Admin, and in effect at once; a Dean or Admin may confirm an automatic one. A course may have several codes, because a reset Classroom link gets a new one. |
...
| **AllowedAdmin** | разрешённый Админ | A Control Plane record: an email permitted to be Admin of one `Installation`. Several per installation are allowed; the email is in that installation's domain (BR-079). |
