...
/// <summary>
/// US-001 db-design §1, §6, US-002 db-design §7 and US-003 db-design §7: the Control Plane migrations in order, and model drift (PC-2).
/// </summary>
public sealed class MigrationTests(PostgreSqlFixture database)
{
    [Fact]
    public async Task Migrations_CreateOwnerAuditEventInstallationAndAllowedAdmin_InOrder()
    {
        ...
        Assert.Equal(new[] { "__EFMigrationsHistory", "allowed_admin", "audit_event", "installation", "owner" }, tables.Order(StringComparer.Ordinal));
