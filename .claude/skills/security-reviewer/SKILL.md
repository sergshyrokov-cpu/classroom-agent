...
is the last automated check before the final commit.
...
| SC-2 Authentication | Identity hashes Dean and Owner passwords; an Admin has no local password, column or reset flow; a Dean login is a domain email; first-run setup requires the one-time code, printed to the console only. Each matches SC-2: the Owner login format; password length and no composition rules; "contains the login" including the email local part and the short-login exception; lockout duration, counter reset and no permanent lockout; one refusal message; the Dean sign-in follows the SC-2 check sequence step by step — lockout before password, disabled only with the correct password, neither counting nor resetting the counter, before any forced password change — and is not built on `PasswordSignInAsync`, `CheckPasswordSignInAsync` or a disabled state expressed through `CanSignInAsync` or Identity lockout; the password an Admin sets on creation and on reset is temporary and the new one differs from it; a reset clears a lockout and never re-enables a disabled account; cookie attributes; HTTPS redirection and HSTS per host and port. |
...
| SC-4 Authorization | Every endpoint and page declares a policy; a fallback policy requires an authenticated user; anonymous access only for the closed list, the fallback catch-all included; one error page per host for `400` antiforgery, `403`, `404` and `500`, and the API-6 body instead under `/api/v1`; private routes answer only on the private local port (DC-6); antiforgery validation is global for POST, PUT, PATCH and DELETE, a refusal is the translated error page or the API-6 body, and exemptions match the closed exemption list; no state-changing action on GET other than the Google OAuth callback. |
...
| SC-9 Channel | A `WorkspaceConnection` whose domain differs from the `Installation` domain is refused; the service channel and Control Plane are not publicly reachable; the installation's private port listens only on its configured address, "all addresses" only as the explicit value `*` (v75, v76); a push only triggers a legitimacy check and never writes the status, at most once a minute with at most one pending check (v77), `404` for another installation's id (v76). |
