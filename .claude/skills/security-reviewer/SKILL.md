...
| SC-9 Channel | A `WorkspaceConnection` whose domain differs from the `Installation` domain is refused; the service channel and Control Plane are not publicly reachable; the installation's private port listens only on its configured address, "all addresses" only as an explicit value (v75). |
