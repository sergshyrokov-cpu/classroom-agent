...
| SC-2 Authentication | Identity hashes Dean and Owner passwords; an Admin has no local password, column or reset flow; a Dean login is a domain email; first-run setup requires the one-time code, printed to the console only. Each matches SC-2: the Owner login format; password length and no composition rules; "contains the login" including the email local part and the short-login exception; lockout duration, counter reset and no permanent lockout; one refusal message; a disabled Dean with the correct password gets its own message, only without a lockout in force, neither counting nor resetting the counter, and before any forced password change; the password an Admin sets on creation and on reset is temporary and the new one differs from it; a reset clears a lockout and never re-enables a disabled account; cookie attributes; HTTPS redirection and HSTS per host and port. |
